"""Factory for consistent CRUD routers for every master entity.

One definition per entity keeps list/search/filter/pagination/create/update/
delete uniform and permission-gated (`ams.<entity>.<action>`).
"""
...
def make_crud_router(
    *,
    ...
    def _require(action):
        ...
        return require_permission(f"ams.{permission}.{action}")
...
def _sqlexc_message(exc) -> str:
    ...
    if isinstance(exc, Exception):
        # psycopg2 / sqlite 3 messages are long; keep the first meaningful line
        first = msg.split("\n")[0]
